...
- Control channel signaling and optional text fallback (`/msg`)
...
```powershell
python -m voicechat.server --host 0.0.0.0 --signal-port 8765 --udp-port 9999
```
...
```powershell
python -m voicechat.server --host 127.0.0.1 --signal-port 8765 --udp-port 9999
```

Clients on the same machine:

```powershell
python -m voicechat.client --server 127.0.0.1 --signal-port 8765 --udp-port 9999 --username alice --room room1
python -m voicechat.client --server 127.0.0.1 --signal-port 8765 --udp-port 9999 --username bob --room room1
```
...
```powershell
python -m voicechat.client --server <SERVER_IP> --signal-port 8765 --udp-port 9999 --username alice --room room1 --token my-secret
```

Second user:

```powershell
python -m voicechat.client --server <SERVER_IP> --signal-port 8765 --udp-port 9999 --username bob --room room1 --token my-secret
```
...
source .venv/bin/activate
python -m voicechat.server --mode public --signal-port 8765 --udp-port 9999
```
...
```powershell
python -m voicechat.client --server <VPS_PUBLIC_IP> --signal-port 8765 --udp-port 9999 --username alice --room global-room --token my-secret
```

```powershell
python -m voicechat.client --server <VPS_PUBLIC_IP> --signal-port 8765 --udp-port 9999 --username bob --room global-room --token my-secret
```
...
- Open firewall ports for both control-channel TCP and UDP relay.
