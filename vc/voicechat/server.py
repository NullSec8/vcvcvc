...
class VoiceRelayServer:
    def __init__(self, host: str, ws_port: int, udp_port: int) -> None:
        self.host = host
    ...
    async def run(self) -> None:
        ...
        async with websockets.serve(self.handle_ws, self.host, self.ws_port):
            banner("Terminal Voice Chat Relay Server")
            info(f"Signaling: ws://{self.host}:{self.ws_port}")
            info(f"UDP relay: {self.host}:{self.udp_port}")
            for hint in _join_hints(self.host, self.ws_port):
                info(hint)
...
def _join_hints(host: str, ws_port: int) -> list[str]:
    hints: list[str] = []
    if host == "127.0.0.1":
        hints.append(f"Clients on this PC can join with: --host 127.0.0.1 --ws-port {ws_port}")
        return hints

    if host == "0.0.0.0":
        hints.append("Public mode enabled: remote users can join using your VPS public IP.")
        hints.append("Open firewall ports: TCP ws-port and UDP udp-port.")
        try:
            lan_ip = socket.gethostbyname(socket.gethostname())
            if lan_ip and lan_ip != "127.0.0.1":
                hints.append(f"LAN example: --host {lan_ip} --ws-port {ws_port}")
        except OSError:
            pass
    return hints
...
def ensure_server_config(args: argparse.Namespace) -> None:
    ...
    if args.ws_port is None:
        args.ws_port = _prompt_int("WebSocket port", 8765)
...
async def main() -> None:
    ...
    server = VoiceRelayServer(args.host, args.ws_port, args.udp_port)
    try:
        await server.run()
        ...
        if "10048" in message or "Address already in use" in message:
            ...
            info(f"Try: python3 server.py --mode public --ws-port {args.ws_port + 1} --udp-port {args.udp_port + 1}")
            return
