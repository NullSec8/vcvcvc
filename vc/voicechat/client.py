...
class VoiceClient:
    ...
    async def run(self) -> None:
        ...
        info(f"Server: {self.args.server}:{self.args.ws_port} (UDP {self.args.udp_port})")
    ...
    async def _run_connected(self) -> None:
        ws_url = f"ws://{self.args.server}:{self.args.ws_port}"
    ...
    async def _text_input_loop(self) -> None:
        ...
        while True:
            ...
            if line == "/status":
                info(
                    f"Status | server={self.args.server}:{self.args.ws_port}, "
